...
Suppose a noisy channel in which, with probability $p$, a bit that passes through flips. This implies that we will only get the correct information with probability $(1 - p)$. We can obtain better results by encoding the information. In this case, suppose we are taking a single bit of information and encoding it with three additional bits. A particular aspect of this model to consider is that every bit in that sequence holds the same amount of information. This is a strictly classical concept called *redundancy*.

```{figure} ../images/repetition_code/01_repetition_code/01_repetition_code.png
:label: img_1
:alt: Representation of the repetition code. It shows 0 with an arrow to 000 and 1 with an arrow to 111.
:align: center

Representation of the repetition code. This image was inspired by the book [@Nielsen2010]. It shows that each bit is represented by three bits—a strictly classical form of redundancy.
```
...
1. We could build a majority-vote system, meaning we interpret the bit string as representing bit $0$ if the majority of elements are $0$, and similarly for $1$.
2. The other decoding method is to check if two bits are equal. This detects that an error occurred and indicates how to correct it. In this particular case, you can compare the first and second bits, and the first and third bits.
...
```{figure} ../images/repetition_code/01_repetition_code/02_comparison.png
:label: img_2
:alt: Graph comparing the probability of error with and without the code. Two lines are shown: one linear increase for p, and one for 3p^2 - 2p^3 that deviates from linearity, being lower for p < 1/2 and higher afterward.
:align: center

Comparison of the probability of error for a bit transmitted through the channel with and without the code. The dashed blue line represents the unprotected bit, whose error probability is linear. The red line represents the coded system, which performs better when the error probability is less than 50\%.
```
